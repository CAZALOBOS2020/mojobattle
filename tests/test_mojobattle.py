...
class TestPasswordCoin:
    ...
    async def test_create_and_spend_password_coin(self, setup):
        ...
        try:
            ...
            password='vega'
            masterwallet= bob.puzzle_hash
            mywallet=alice.puzzle_hash
            attack=1
            opattack=2
            ...
            password_coin: Optional[CoinWrapper] = await alice.launch_smart_coin(
                create_coin_puzzle(
                    create_coin_password_hash_from_string(password),masterwallet,mywallet,int(attack)),
                amt=100
            )
            ...
            password_spend: SpendBundle = await alice.spend_coin(
                ...
                args=solution_for_password(password,int(100),mywallet,int(opattack)),
            )
            ...
            filtered_result: List[Coin] = list(
                filter(
                    lambda addition: (addition.amount == 1) and (
